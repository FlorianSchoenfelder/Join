...
function countAllTasks() {
  ...
  let totalTasks = toDoTasks.length + inProgressTasks.length + awaitFeedbackTasks.length + doneTasks.length;
...
function countPriorityZeroTasks() {
  let toDoTasks = contactData[0].tasks[0].toDo.filter(task => task.prio === 0);
  let inProgressTasks = contactData[0].tasks[0].inProgress.filter(task => task.prio === 0);
  let awaitFeedbackTasks = contactData[0].tasks[0].awaitFeedback.filter(task => task.prio === 0);
  let doneTasks = contactData[0].tasks[0].done.filter(task => task.prio === 0);

  let totalPriorityZeroTasks = toDoTasks.length + inProgressTasks.length + awaitFeedbackTasks.length + doneTasks.length;
...
function findClosestDueDatePrioZero() {
  ...
  let allPrioZeroTasks = [...toDoTasks, ...inProgressTasks, ...awaitFeedbackTasks, ...doneTasks];
